...
void getColor(int m, double val, int *re, int *gr, int *bl)
{
	...
	//input value val is between 0 and 1, make it to 0 and 255
	val = val * 255;
	fact = (255-val)/255.00;
...
// helper functions
// if one of the four values in one region is not valid (i.e. -9999)
// then the whole region is not valid (set all four values be -9999)
...
int main(int argc, char* argv[])
{
	...
	for(i=0; i < size1; i++)
	{
		...
		for(j=0;j < size2; j++)
		{
			// if the data is not valid (i.e. -9999), output color be grey (i.e. (r,g,b)=(125,125,125))
			if(arr[i][j] < 0)
			{
				outf<<125 <<" "<<125 <<" "<<125 <<" ";
			}
